...
| 2   | js-exercises/admin-portal    | moefingers/JS-Building-an-Admin-Portal                       | A (static)                   | · `/js-exercises/admin-portal?page=admin` 🔴<br>· `/api-client?api=admin-portal` 🔴                       |  🔴  | 🔴  | 🔴  |   🔴   | 🔴  | Three-page (Admin, Books, API Docs); paired with `admin-portal-api` (row 10) via `siblingApiClient`. Hosting fixes: db.json cleanup + localhost:3001→/api/admin-portal URL rewrites. Routes column mirrors the pair — the api-route's status is authoritatively owned by row 10 but echoed here for at-a-glance pair visibility.            |
| 3   | js-exercises/shared-counter  | moefingers/JS-Building-a-Shared-Counter-Part-1               | A (static)                   | · `/js-exercises/shared-counter` 🔴                                                                       |  🔴  | 🔴  | 🔴  |   🔴   | 🔴  |                                                                                                                                                                                                                                                                                                                                             |
| 4   | js-exercises/interactive-map | moefingers/JS-Making-an-Interactive-Map                      | A (static)                   | · `/js-exercises/interactive-map` 🔴                                                                      |  🔴  | 🔴  | 🔴  |   🔴   | 🔴  | Hosting fixes: 3s geolocation timeout + LV fallback, Foursquare key redacted (button no-ops gracefully).                                                                                                                                                                                                                                    |
| 5   | jaskis                       | moefingers/API-JASKIS                                        | E (backend-only)             | · `/api-client?api=jaskis` 🔴                                                                             |  🔴  | 🔴  | 🔴  |   🔴   | 🔴  | Backend-only — no frontend half (no `siblingFrontend`). Lives under `/api-client` exclusively.                                                                                                                                                                                                                                              |
...
| 10  | admin-portal-api             | (paired with admin-portal)                                   | (api-client entry)           | · `/js-exercises/admin-portal?page=admin` 🔴<br>· `/api-client?api=admin-portal` 🔴                       |  🔴  | 🔴  | 🔴  |   🔴   | 🔴  | Backend half of admin-portal pair. Source-side metadata flows through admin-portal's row (row 2). This row authoritatively owns the api-route's ✓ status; the frontend-route status is echoed from row 2 for at-a-glance pair visibility. `siblingFrontend` not surfaced today (api-client UI doesn't render the reverse cross-link yet).   |
...
| 33  | sql-injection-demo-api       | (paired with sql-injection-demo)                             | (api-client entry)           | · `/sql-injection-demo` 🔴<br>· `/api-client?api=sql-injection-demo` 🔴                                   |  ➖  | ➖  | ➖  |   🔴   | 🔴  | Backend half of sql-injection-demo pair. Source-side metadata flows through #7's row. This row authoritatively owns the api-route's ✓ status; the frontend-route status is echoed from row 7.                                                                                                                                               |
