/**
 * /api/v2/admin-portal/audit-log — Enhanced-tier observability surface.
 *
 *   GET /audit-log?op=&actor=&since=&tier=&limit=
 *
 * Returns the admin_portal.audit_log rows, newest first. Every mutation
 * on /api/admin-portal/* (Original) and /api/v2/admin-portal/* (Enhanced)
 ...
 *
 * The `table` filter that music-tour exposes is omitted here — there's
 * only one collection in admin_portal (`books`), so the filter would
 * have no effect.
 *
...
import { auditLog } from "../../../admin-portal/_schema";
